...
async function loadElectionsHistory() {
  try {
    ...
    _currentCongressElectionId = congressElections.length ? congressElections[congressElections.length - 1]._id : null;

    const sortedPres = [...presidentialElections].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    _latestPresidentialElectionId = sortedPres[0]?._id || null;
...
function renderTimeline(congressElections) {
  ...
  safeDestroy('timelineChart');
    // Forza la distruzione di qualsiasi chart residuo
  if (window._timelineChart) {
    window._timelineChart.destroy();
    window._timelineChart = null;
  }
...
async function loadTimelineData(elections, electionIds) {
  ...
  for (const pid of allPids) {
    ...
    const pointRadii = electionIds.map(eid => (eid === _currentCongressElectionId) ? 7 : 5);
    datasets.push({
  ...
  if (_timelineChart && datasets.length) {
    ...
    if (filterSelect) {
      ...
      filterSelect.onchange = () => {
        ...
        _timelineChart.data.datasets.forEach(ds => {
        ...
        _timelineChart.data.datasets.forEach((ds, i) => {
          if (!ds.label && i > 0) {
            const prev = _timelineChart.data.datasets[i - 1];
...
function updateTimelineHighlight() {
  ...
  _timelineChart.data.datasets.forEach(dataset => {
    ...
    dataset.pointRadius = _timelineElectionIds.map(eid => (eid === _currentCongressElectionId) ? 7 : 4);
...
async function getGovernmentWithDetails(countryId) {
...
function renderCharts(electedParties) {
  ...
  _seatsChart = new Chart(document.getElementById('seatsChart').getContext('2d'), {
    ...
    options: {
      ...
      onClick: (evt, el) => {
        ...
        const meta = _seatsChart.getDatasetMeta(0);
  ...
  if (votesCanvas && totalVotes > 0) {
    ...
    _votesChart = new Chart(votesCanvas.getContext('2d'), {
      type: 'bar',
      data: {
...
function renderAllPartiesChart(allParties) {
  ...
  _allPartiesChart = new Chart(canvas.getContext('2d'), {
    type: 'bar',
    data: {
...
async function loadCongressElection(election) {
    ...
    if (countdownEl) {
      if (isOngoing) {
        ...
        const tick = () => {
          ...
          if (remaining <= 0) { clearInterval(_congressCountdownInterval); countdownEl.textContent = 'Voting ended'; loadElection(election._id); return; }
        ...
        tick();
        _congressCountdownInterval = setInterval(tick, 1000);
      } else {
  ...
  if (refreshBtn) refreshBtn.onclick = () => { if (election._id) loadElection(election._id); };
...
function renderSimulator(allParties, totalSeatsCurrent) {
  ...
  // Historic chips
  const recentTurnouts = _electionHistory.filter(e => e.type === 'congress').slice(-6).map(e => ({
  ...
  if (refEl) {
    if (!recentTurnouts.length) {
      ...
      refEl.innerHTML = recentTurnouts.map((t, i) => {
        ...
        return `<span class="sim-chip${last ? ' sim-chip-latest' : ''}" title="${d}: ${t.totalVotes.toLocaleString()} votes">${d} · ${pct}</span>`;
      }).join('');
    }
  }
  ...
  const partiesWithVotes = allParties.filter(p => (p.votes || 0) > 0);
  ...
  // Mini parliament preview (only if Parliament module available)
  ...
  if (previewWrap && previewContainer && simHasSeats && typeof Parliament !== 'undefined') {
...
function _fillStatTrends(currentSeats, currentParties, currentVotes) {
  ...
  const congHistory = _electionHistory.filter(e => e.type === 'congress');
  ...
  const cachedPrevParties = window._prevCongressPartyCount || null;
  setTrend('stat-parties-trend', currentParties, cachedPrevParties, v => v);
  window._prevCongressPartyCount = currentParties;
}
