...
async function loadPresidentialElection(election, isLatestPresidential) {
  ...
  await Promise.all(candidatePartyIds.map(async pid => {
    try {
      ...
      if (p?.name) candPartyMap[pid] = { name: p.name, color: _csvColorMap.get(pid) || PALETTE[Object.keys(candPartyMap).length % PALETTE.length] };
    } catch (_) {}
  }));
  ...
  if (now <= end && now >= start) {
    const tick = () => {
      ...
      if (remaining <= 0) { clearInterval(window._presCountdown); if (countdownEl) countdownEl.textContent = 'Voting ended'; return; }
    ...
    tick();
    window._presCountdown = setInterval(tick, 1000);
  } else {
...
function renderPresidentialTurnoutChart(currentElectionId = null) {
  const presidentialElections = _electionHistory.filter(e => e.type === 'president').sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    ...
    options: {
      ...
      onClick: (event, elements) => {
        if (elements.length > 0) {
          ...
          if (eid) { document.getElementById('electionSelect').value = eid; document.getElementById('electionIdInput').value = eid; loadElection(eid); }
        }
      },
    },
  });
...
function onPresSimInput() {
  if (window._lastPresData) renderPresSimulator(window._lastPresData.candidates, window._lastPresData.totalVotes, window._lastPresData.election);
}
...
async function renderPresidentialHistoricWinners(currentElectionId) {
  ...
  const presidentialElections = _electionHistory.filter(e => e.type === 'president').sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
