/* ── SENATE VIEW ── horizontal semicircle (left→right, arc up) + fixed right sidebar + bottom bar ── */
const SenateView = (() => {
  // ── state ──
  let _parties = [];
  let _filtered = [];
  ...
  let _govData = null;        // { presidentData, vicePresidentData, ... } per _currentCountryId
  ...
  // NB: NON dichiariamo qui una copia locale di _currentCountryData: quella
  // variabile esiste già globale (config.js) e viene tenuta sincronizzata da
  // main.js sia al boot sia ad ogni cambio nazione. Duplicarla qui con `let`
  // la ombreggerebbe silenziosamente, disallineando il senato dal resto
  // dell'app. Leggiamo quindi sempre quella globale.
  ...
  const overlay = document.getElementById('senateOverlay');
  const seatsContainer = document.getElementById('senateSeats');
  const emptyMsg = document.getElementById('senateEmpty');
  const subtitle = document.getElementById('senateSubtitle');
    ...
    for (const pt of points) {
      {
        ...
        card.addEventListener('click', () => {
          if (seat.userId && typeof openPlayerCard === 'function') openPlayerCard(seat);
        });
  ...
  function _countryCode(country) {
    if (!country) return _countryCodesMap.get(_currentCountryId) || '';
    const raw = country.code || country.countryCode || country.iso || country.isoCode || country.abbr || country.abbreviation || _countryCodesMap.get(_currentCountryId) || '';
  ...
  function _flagMarkup(country) {
    ...
    const nameForInitials = country?.name || _countryNamesMap.get(_currentCountryId) || '?';
  ...
  function _govAvatar(user, { big = false } = {}) {
    if (!user) return '';
    const sizeClass = big ? 'scb-gov-avatar scb-gov-avatar-president' : 'scb-gov-avatar';
  ...
  function _renderCountryBox(seats) {
    ...
    if (!box) return;
    const countryName = _countryNamesMap.get(_currentCountryId) || _currentCountryId || '—';
    const totalVotes = _parties.reduce((s, p) => s + (p.votes || 0), 0);
    const totalSeats = seats.length;
    const factionsInSenate = new Set(seats.map(s => s.partyId)).size;

    const c = (_currentCountryData && _currentCountryData._id === _currentCountryId) ? _currentCountryData : null;
    ...
    const rulingPartyName = c?.rulingParty ? (_partyNamesMap.get(c.rulingParty) || null) : null;
    const totalParties = (_totalPartiesCountryId === _currentCountryId) ? _totalPartiesCount : null;
    ...
    // quando _currentCountryData non ha ancora un certo campo.
    const extraStats = [];
  ...
  function _injectStyles() {
    ...
    style.textContent = `
      ...
      .sps-empty { margin-top:40%; text-align:center; color:#5f6a80; font-size:.82rem; }
      .sps-party {
        margin-bottom:16px; padding-bottom:14px; border-bottom:1px solid rgba(255,255,255,.06);
      }
      .sps-party:last-child { border-bottom:none; }
      .sps-party-header { display:flex; align-items:center; gap:8px; }
      .sps-dot { width:10px; height:10px; border-radius:50%; flex:0 0 auto; box-shadow:0 0 8px -1px var(--pc); }
      .sps-party-name {
        font-size:.8rem; font-weight:700; color:#f2e6c8; flex:1; min-width:0;
        white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
      }
      .sps-party-seats { font-size:.68rem; font-weight:700; color:#e8c97a; flex:0 0 auto; }
      .sps-party-votes { font-size:.66rem; color:#8892a4; margin:3px 0 8px 18px; }
      .sps-members { display:flex; flex-direction:column; gap:4px; }
      .sps-member {
      ...
      .sps-member:hover { background: rgba(255,255,255,.06); }
      .sps-avatar {
      ...
      .sps-member-name { font-size:.72rem; flex:1; min-width:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .sps-member-votes { font-size:.7rem; font-weight:700; color:#e8c97a; flex:0 0 auto; }
      .sps-member-pct { font-size:.62rem; color:#8892a4; flex:0 0 auto; width:34px; text-align:right; }
      ...
      .ssb-chip { white-space:nowrap; }
      .ssb-chip b { color:#e8c97a; }
      .ssb-track { flex:1; height:10px; border-radius:5px; overflow:hidden; display:flex; box-shadow: inset 0 0 0 1px rgba(255,255,255,.08); }
      .ssb-seg { height:100%; transition: width .5s ease; }
      ...
      .scb-top { display:flex; align-items:center; gap:10px; }
      .scb-flag-img { width:40px; height:28px; object-fit:cover; border-radius:4px; box-shadow:0 0 0 1px rgba(255,255,255,.15); flex:0 0 auto; }
      .scb-flag-fallback {
      ...
      .scb-stats-extra { padding-top:8px; border-top:1px solid rgba(197,150,74,.15); }
      .scb-stat { display:flex; flex-direction:column; min-width:0; }
      .scb-stat-val { font-size:1rem; font-weight:700; color:#e8c97a; line-height:1.2; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:110px; }
      .scb-stats-extra .scb-stat-val { font-size:.86rem; }
      .scb-stat-label { font-size:.62rem; color:#8892a4; text-transform:uppercase; letter-spacing:.03em; }
      ...
      @media (max-width: 1000px) {
        ...
        .senate-table { width: 100%; border-collapse: collapse; }
        .senate-table thead th {
  ...
  // Riusano lo stato globale dell'app (_currentCountryId, loadElection, ecc.)
  ...
  async function _populateSenateSelectors() {
    ...
    if (!countrySel._loaded) {
      try {
        ...
        items.forEach(c => {
          _countryNamesMap.set(c._id, c.name);
    ...
    if (countrySel._loaded) countrySel.value = _currentCountryId;

    await _populateSenateElections(_currentCountryId, electionSel);
  }
  ...
  function _electionSortKey(e) {
    const t = new Date(e.votesStartAt || e.votesEndAt || e.createdAt || 0).getTime();
    return isNaN(t) ? 0 : t;
  }
  ...
  async function _populateSenateElections(countryId, electionSel) {
    ...
    try {
      ...
      const keepCurrent = countryId === _currentCountryId && _currentCongressElectionId &&
        congress.some(e => e._id === _currentCongressElectionId);
      electionSel.value = keepCurrent ? _currentCongressElectionId : (congress[0]?._id || '');
      return congress;
    } catch (err) {
  ...
  // window._lastElectedParties: qui basta poi ridisegnare l'arco con quello.
  async function _loadSenateElection(electionId) {
    ...
    try {
      ...
      render(window._lastElectedParties || []);
    } catch (err) {
  ...
  async function _onSenateCountryChange(newCountryId) {
    if (!newCountryId || newCountryId === _currentCountryId) return;
    ...
    try {
      ...
      const codeAlias = _currentCountryData?.code || _currentCountryData?.countryCode || _currentCountryData?.iso || _currentCountryData?.isoCode || _currentCountryData?.abbr || _currentCountryData?.abbreviation;
  ...
  // appoggia alla stessa variabile globale _currentCountryData usata dal
  ...
  async function _ensureCurrentCountryData() {
    if (typeof _currentCountryData !== 'undefined' && _currentCountryData?._id === _currentCountryId) return;
    try {
      const data = await localFetch('/countries', {}, { useCache: false });
      const items = data?.items || [];
      items.forEach(c => _countryNamesMap.set(c._id, c.name));
      _currentCountryData = items.find(c => c._id === _currentCountryId) || null;
    } catch (err) {
  ...
  // già definita in congress.js (globale, caricata dopo senate.js ma sempre
  // pronta a runtime, ben prima che l'utente possa aprire il pannello).
  async function _ensureGovernment() {
    if (_govDataCountryId === _currentCountryId && _govData !== null) return;
    _govDataCountryId = _currentCountryId;
    try {
      _govData = typeof getGovernmentWithDetails === 'function'
        ? await getGovernmentWithDetails(_currentCountryId)
        : null;
    } catch (err) {
  ...
  // che come effetto collaterale popola anche _partyNamesMap per TUTTI i
  // partiti del paese (utile anche per risolvere il nome del ruling party).
  async function _ensureTotalPartiesCount() {
    if (_totalPartiesCountryId === _currentCountryId && _totalPartiesCount !== null) return;
    _totalPartiesCountryId = _currentCountryId;
    try {
      const parties = typeof loadPartiesForCountry === 'function'
  ...
  function open() {
    ...
    if (!window._senateResizeListener) {
      window._senateResizeListener = true;
      let timer = null;
