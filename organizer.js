...
function initMemberOrganizer(members, partyId) {
  ...
  // FIX: autoBtn era fuori dalla funzione — ora correttamente dentro
...
function autoAssignSupporters(members, partyId, minVotesToWin) {
  ...
  const groupsNeeds = groupsWithCandidate.map(g => ({
    group: g,
    needed: Math.max(0, minVotesToWin - g.supporterIds.length),
  }));
    ...
    const candidateCard = candidate ? `
      <div class="candidate-card">
        ...
        <div class="candidate-stats">
          <span class="supporter-count">${supporters.length} / ${_lastMinVotesToWin}</span>
