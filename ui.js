...
function showPartyView() {
  ...
  if (_currentPartyId) loadPartyDetails(_currentPartyId);
}
function showElectionView() {
  document.getElementById('party-view').style.display = 'none';
...
function showSkeletonPartyView() {
  const blanks = {
...
function hideSkeletonPartyView() {
...
async function loadCountries() {
  ...
  try {
    ...
    tomSelect.setValue(savedOption ? _currentCountryId : (Object.keys(tomSelect.options)[0] || ''));
